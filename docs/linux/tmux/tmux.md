...
在 tmux 中，使用快捷键是使用 `ctrl+B`/`control+B` 去唤起快捷键的，叫**前缀键**

> [!NOTE|label:很重要的快捷键操作指南]
> 例如挂起 session 的快捷键是 `ctrl+B d` 。意思是，先按下前缀键 `ctrl+B`/`control+B` 唤起快捷键操作，然后接着按下 `d` 进行相应操作
...
tmux detach
```
这时候也可以用快捷键 `ctrl+B d` 来操作

这时候该 session 会继续挂在后台运行继续执行任务，直到再次被唤起或被 kill 掉。
